...
export default {
  ...
  methods: {
    initmap() {
      //加载图例图片
      let url='http://localhost:8081/geoserver/wms?REQUEST=GetLegendGraphic&VERSION=1.1.0&FORMAT=image/png&WIDTH=40&HEIGHT=60&LAYER=yyc:panel_xiaoqu';
      ...
      this.map.on('singleclick', function(evt) {
        ...
        for(var i=0; i<layers.length;i++){
          if(layers[i].getProperties()['name'] && layers[i].getProperties()['name']=="xiaoquWMSLayer"){
          ...
          _this.$axios.get(featureUrl).then(res=>{
            if (res.data.features) {
              ...
              if (feature) {
                ...
                popupLayer.setPosition([property.geometry.flatCoordinates[0],property.geometry.flatCoordinates[1]]);
              } else {
                console.log("查询失败");
              }
            }
